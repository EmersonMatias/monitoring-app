...
import { useFindUniqueAgencyFilter } from "@/hooks/hooks-agency"
import queryString from "query-string"
import { convertTimeToBrasilia, formatDateToBR } from "@/functions/functions"

export default function AgencyReport({ params: { id } }: { readonly params: { id: string } }) {
    const [initialDate, setInitialDate] = useState('')
    const [finalDate, setFinalDate] = useState('')

    const { data: agency, isSuccess, isPending } = useFindUniqueAgencyFilter(id, initialDate, finalDate)
            ...
            {isSuccess &&
                <div ref={contentDocument} className={`bg-white mt-20 flex flex-col items-center pb-10 pt-10 border-[2px] rounded-2xl`} >
                    ...
                    <table className="w-1/2 table-auto mb-20 text-center">
                        ...
                        <tbody>
                            {
                                agency?.Checkpoint?.map((checkpoints) => (
                                    <tr key={checkpoints.id}>
                                        ...
                                        <td className="  px-4 py-5 max-w-[200px] border-y-slate-300 border-y-2"> {convertTimeToBrasilia(checkpoints?.user?.entryTime)}</td>
                                        <td className="  px-4 py-5 max-w-[200px] border-y-slate-300 border-y-2"> {convertTimeToBrasilia(checkpoints?.user?.departureTime)}</td>
