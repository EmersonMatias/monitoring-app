...
export default function Header() {
    ...
    function alertsUnviewed() {
        if (alertSuccess) {
            return alerts?.filter((alert) => alert?.viewed === false).length
        }

        return 0
    }

    function alertsUnviewedMessage() {
        if (alertsUnviewed() === 0) {
            return "hidden"
        }
    }
    ...
    return (
        <header className="h-20 bg-[#f0a830] flex items-center justify-center w-[100%]">
            ...
            <div className="flex gap-20">
                ...
                <Link href="/contingencia" className="relative">
                    <div className={`text-white rounded-full w-[30px] h-[30px] flex justify-center items-center font-bold bg-red-500 absolute left-[-18px] top-[-8px] bg-opacity-80 ${emergencyStatusContingency === 0 && "hidden"}`}>
                        {emergencyStatusContingency}
                    </div>

                    <svg className="w-[40px] h-[40px] text-gray-800 dark:text-white" fill="currentColor" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="absolute right-20 flex gap-8">
                ...
                <div className="relative">
                    <div className={`text-white rounded-full w-[30px] h-[30px] flex justify-center items-center font-bold bg-red-500 absolute left-[-10px] top-[-10px] bg-opacity-80 ${alertsUnviewedMessage()} `}>
                        {alertsUnviewed()}
                    </div>
